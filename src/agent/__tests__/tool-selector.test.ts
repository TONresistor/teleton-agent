...
describe("provider tool limits", () => {
  ...
  it("keeps discovery ahead of ordinary schemas", () => {
    const limited = enforceProviderToolLimit(
      [tool("ordinary_a"), tool("ordinary_b"), tool("tool_search")],
      2
    );
    expect(limited.map((entry) => entry.name)).toEqual(["tool_search", "ordinary_a"]);
  });
