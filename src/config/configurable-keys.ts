...
export const CONFIGURABLE_KEYS: Record<string, ConfigKeyMeta> = {
  ...
  "telegram.dm_debounce_ms": {
    type: "number",
    category: "Telegram",
    label: "DM Debounce (ms)",
    description:
      "Direct-message debounce window in ms. Rapid DMs are merged into one context-aware reply (0 = disabled)",
    sensitive: false,
    hotReload: "instant",
    validate: numberInRange(0, 10000),
    mask: identity,
  ...
  "telegram.reaction_events": {
    type: "boolean",
    category: "Telegram",
    label: "Reaction Events",
    description: "Let the agent receive reactions to its messages (bot mode only)",
    sensitive: false,
    hotReload: "instant",
    validate: enumValidator(["true", "false"]),
    mask: identity,
  ...
  "telegram.reply_style": {
    type: "enum",
    category: "Telegram",
    label: "Reply Style",
    description: "How outgoing messages should use Telegram replies",
    sensitive: false,
    hotReload: "instant",
    options: ["auto", "plain", "reply"],
    optionLabels: {
      auto: "Auto",
      plain: "Plain",
      reply: "Reply",
    },
    validate: enumValidator(["auto", "plain", "reply"]),
    mask: identity,
    parse: identity,
  },
  ...
  "heartbeat.proactive_enabled": {
    type: "boolean",
    category: "Agent",
    label: "Proactive Messages",
    description: "Allow heartbeat to initiate messages in approved chats",
    sensitive: false,
    hotReload: "instant",
    validate: enumValidator(["true", "false"]),
    mask: identity,
  ...
  "heartbeat.proactive_cooldown_ms": {
    type: "number",
    category: "Agent",
    label: "Proactive Cooldown",
    description: "Minimum milliseconds between proactive messages per chat",
    sensitive: false,
    hotReload: "instant",
    validate: numberInRange(3_600_000, 31_536_000_000),
    mask: identity,
    parse: (v) => Number(v),
  },
